...
## ADR-010 제출 스킬의 필수 입력 질의 + 자동 유추 확인 흐름 도입

- 상태: 승인
- 날짜: 2026-02-20

### 배경
- 제출 자동화 스크립트는 기존에 필수 인자를 CLI로 모두 강제해, 참가자가 어떤 항목을 반드시 확인해야 하는지 대화 흐름이 분명하지 않았습니다.
- 프로젝트 문맥(README, git remote, package metadata)에서 일부 값을 유추할 수 있음에도, 사용자가 이를 검증하는 절차가 스킬 레벨에 명시되지 않았습니다.
- 해카톤 제출 공정성 관점에서 필수 입력값 수집 근거와 사용자 확인 로그를 명확히 남길 필요가 있었습니다.

### 결정
- `hackathon-submission` 스킬 문서(`SKILL.md`)에 필수 7개 항목 질의 템플릿을 명시하고, 자동 유추값은 반드시 사용자 확인 후 확정하도록 규정합니다.
- `create_submission_pr.py`에 프로젝트 컨텍스트 기반 유추 로직을 추가합니다.
  - 추정 대상: 팀명, 프로젝트명, 저장소 URL, 데모 URL/실행 방법, 문제 정의, 한 줄 소개, 팀 소개/역할
- 스크립트는 대화형 TTY에서 필수 7개 항목을 질문하고 확인 절차를 수행합니다.
- `--non-interactive` 모드에서는 필수값 누락 시 생성 중단 + 누락/추정값 안내로 실패 처리합니다.

### 근거
- 필수값 질의와 최종 확인 단계를 명시하면 제출 데이터의 누락/오입력 위험을 낮출 수 있습니다.
- 자동 유추는 입력 부담을 줄이되, 사용자 검증 단계를 강제해 잘못된 추정값 전파를 방지합니다.
- 비대화형 모드의 엄격 실패 정책은 CI/자동화 환경에서 입력 책임을 명확히 합니다.

### 결과
- 제출 실행 전 필수 7개 항목 수집/검증 흐름이 스킬 문서와 스크립트 동작 모두에서 일관되게 적용됩니다.
- 유추 가능한 정보는 기본값으로 제안되어 입력 시간이 줄고, 사용자 확인으로 정확도를 보완할 수 있습니다.
