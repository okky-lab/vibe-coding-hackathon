...
## ADR-025 사용자 승인 기반 공식 소스 참조 수동 적용

- 상태: 승인
- 날짜: 2026-02-20

### 배경
- 설치 경로(`@gooseui/footer-simple`, `@gooseui/footer-01`, URL 직접 설치 2건)가 모두 404로 실패해 자동 설치 기반 적용이 불가능했습니다.
- 이후 사용자가 `goosen-x/gooseui`의 `footer-simple.tsx`를 명시적으로 참조하도록 지시했습니다.

### 결정
- 기본 정책(ADR-024)의 중단 규칙은 유지하되, 사용자 명시 승인 시 공식 소스 참조 수동 적용을 예외적으로 허용합니다.
- `src/components/site-footer.tsx`는 GooseUI Simple Footer 구조를 기반으로 재구성합니다.
- 사이트 고유 콘텐츠는 유지합니다.
  - 해카톤 안내 문구
  - `OKKY`, `개요`, `행동강령` 링크

### 결과
- 레지스트리 미배포 상태에서도 사용자 승인 범위 내에서 footer UI를 적용할 수 있게 되었습니다.
- 출처를 공식 저장소로 고정해 수동 적용의 근거와 추적 가능성을 확보했습니다.

## ADR-026 Footer 로고를 네비게이션 로고와 통일하고 중앙 문구 제거

- 상태: 승인
- 날짜: 2026-02-20

### 배경
- Footer에 임시 플레이스홀더/브랜드 텍스트가 남아 있어 네비게이션 로고와 시각적으로 불일치했습니다.
- 사용자 요청으로 Footer 로고를 네비게이션과 통일하고 로고 하단 중앙 문구를 제거해야 했습니다.

### 결정
- `src/components/site-footer.tsx`의 로고를 네비게이션과 동일한 OKKY 이미지 로고로 교체합니다.
  - `https://static.cdn.okky.kr/okky-web/public/okky.png`
- 중앙 소개 문구는 제거합니다.
- 기존 링크(`OKKY`, `개요`, `행동강령`)와 저작권 라인은 유지합니다.
...
## ADR-028 Sponsor 노출을 랜딩에서 전용 페이지로 분리

- 상태: 승인
- 날짜: 2026-02-20

### 배경
- 랜딩 CTA 하단 Sponsor 섹션은 초기 노출 요구를 충족했지만, 메인 메시지 집중도를 낮추는 문제가 있었습니다.
- 사용자 요청으로 Sponsor 영역을 랜딩에서 제거하고, 별도 페이지에서 회사 소개와 함께 제공해야 했습니다.

### 결정
- 랜딩 페이지(`src/app/(site)/page.tsx`)에서 Sponsor 섹션을 제거합니다.
- 전용 페이지 `src/app/(site)/sponsor/page.tsx`를 추가합니다.
- Sponsor 페이지에는 eBrain, PopupStudio 로고와 간략 소개, 외부 링크를 제공합니다.
- 상단 내비게이션에 `스폰서` 메뉴(`/sponsor`)를 추가해 진입 경로를 명시합니다.

### 결과
- 랜딩은 핵심 CTA에 집중하고, Sponsor 정보는 별도 컨텍스트에서 확인할 수 있게 되었습니다.
- 스폰서 로고/소개/링크를 독립 페이지로 제공해 확장성과 유지보수성이 개선되었습니다.

## ADR-029 팀 소개 라우트에 공통 헤더/푸터 레이아웃 적용

- 상태: 승인
- 날짜: 2026-02-20

### 배경
- `/team`과 `/team/[slug]` 페이지는 카드/본문 콘텐츠만 렌더링되어, 사이트 공통 헤더/푸터가 없는 상태였습니다.
- 사용자 요청으로 팀 소개 페이지에도 상단 header와 하단 footer를 동일하게 제공해야 했습니다.

### 결정
- `src/app/team/layout.tsx`를 추가해 팀 라우트 세그먼트에 전용 레이아웃을 적용합니다.
- 해당 레이아웃은 `HomeLayout {...baseOptions()}`와 `SiteFooter` 조합을 사용해 `(site)` 구간과 동일한 헤더/푸터 구조를 제공합니다.
...
## ADR-031 행사 일정 타임라인 상세화 및 종료 시각 연장

- 상태: 승인
- 날짜: 2026-02-21

### 배경
- 운영 확정안 기준으로 `15:30` 이후 세션 구성이 기존 일정 문서와 달라졌고, 발표/심사/시상 구간이 더 세분화되었습니다.
- 기존 일정 문서와 SPEC의 시간 기준(`18:30 이후`, `13:00~18:00`)이 최신 운영안과 불일치했습니다.

### 결정
- `contents/docs/schedule.mdx` 타임테이블의 후반 구간을 아래와 같이 갱신합니다.
  - `15:30 - 17:20`: 구현 & 테스트 (Implementation & Test)
  - `17:20 - 17:30`: 휴식시간 및 팀별 발표 준비
  - `17:30 - 18:45`: 팀별 데모 발표 (팀당 3분, 첫 7건 발표 후 5분 휴식)
  - `18:45 - 18:55`: 참가자 휴식 & 심사위원 심사
  - `18:55 - 19:00`: 시상식 & 클로징
  - `19:00+`: 자유 네트워킹 & 희망자 디너
- 일정 문서 frontmatter의 권장 시간 범위를 `12:30~19:00+`로 조정합니다.
- `SPEC.md`의 `FR-008`, `AC-006` 시간 범위를 `12:30~19:00+`로 정렬합니다.
...
## ADR-034 팀 목록 제출 시각 KST 고정 표기

- 상태: 승인
- 날짜: 2026-02-21

### 배경
- `/team` 카드의 제출 시각은 실행 환경 타임존에 영향을 받을 수 있어, 운영/참가자 간 표시 시각이 다르게 보일 가능성이 있었습니다.
- 행사 운영 시간대는 한국 표준시(KST) 기준 안내가 필요합니다.

### 결정
- `src/app/team/page.tsx`의 제출 시각 포맷 함수에 타임존 `Asia/Seoul`을 고정 적용합니다.
- 제출 시각 문자열 끝에 `KST`를 명시적으로 추가합니다.
